...
class RoleService:
    ...
    def assign_role(
        ...
        if created:
            log_creation(
                actor=assigned_by,
                action="roles.assignment_created",
                instance=user_role,
                reason=reason,
                metadata={
                    "role_slug": role_slug,
            ...
            log_transition(
                actor=assigned_by,
                action="roles.assignment_updated",
                instance=user_role,
    ...
    def approve_role_change(request: RoleChangeRequest, approved_by: User) -> UserRole:
        ...
        old_request = RoleChangeRequest.objects.get(pk=request.pk)
    ...
    def deactivate_role(
        ...
        old_user_role = UserRole.objects.get(user=user, role=role)
        UserRole.objects.filter(user=user, role=role).update(is_active=False)
        updated_user_role = UserRole.objects.get(user=user, role=role)

        log_transition(
            actor=deactivated_by,
            action="roles.assignment_deactivated",
            instance=updated_user_role,
            old_instance=old_user_role,
