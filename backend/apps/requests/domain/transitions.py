...
TRANSITIONS = [
    ...
    Transition(RequestState.AWAITING_PAYMENT, RequestAction.PAYMENT_WEBHOOK, RequestState.AWAITING_ASSIGNMENT, "system.webhook", TriggerType.SYSTEM, lambda ctx: _verified_transaction(ctx) and ctx.requires_technician),
    Transition(RequestState.AWAITING_PAYMENT, RequestAction.PAYMENT_WEBHOOK, RequestState.IN_PROGRESS, "system.webhook", TriggerType.SYSTEM, lambda ctx: _verified_transaction(ctx) and not ctx.requires_technician),
