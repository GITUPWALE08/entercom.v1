...
class TestBookingServices:
    ...
    def test_start_booking_service(self, mock_event, mock_audit):
        """
        ...
        booking = Booking.objects.create(request_id=uuid.uuid4(), technician_id=1, status=Booking.Status.SCHEDULED)
    ...
    def test_extend_duration_service(self, mock_conflict, mock_event, mock_audit):
        mock_conflict.return_value = False
        actor = MagicMock(role=Roles.TECHNICIAN, id=1)
        now = timezone.now()
        booking = Booking.objects.create(request_id=uuid.uuid4(), technician_id=1, status=Booking.Status.IN_PROGRESS, start_time=now, duration_days=1)
    ...
    def test_sync_completion_service(self, mock_event, mock_audit):
        actor = MagicMock(role=Roles.SYSTEM, id="SYSTEM")
        booking = Booking.objects.create(request_id=uuid.uuid4(), technician_id=1, status=Booking.Status.IN_PROGRESS)
    ...
    def test_sync_cancellation_service(self, mock_event, mock_audit):
        actor = MagicMock(role=Roles.SYSTEM, id="SYSTEM")
        booking = Booking.objects.create(request_id=uuid.uuid4(), technician_id=1, status=Booking.Status.SCHEDULED)
    ...
    def test_has_conflict(self):
        ...
        """
        now = timezone.now()
        Booking.objects.create(request_id=uuid.uuid4(), technician_id=1, status=Booking.Status.SCHEDULED, start_time=now, end_time=now + timedelta(hours=2))
        
        # Conflict exists
        assert AvailabilityService.has_conflict("1", now + timedelta(hours=1), now + timedelta(hours=3)) is True
        # Conflict doesn't exist
        assert AvailabilityService.has_conflict("1", now + timedelta(hours=3), now + timedelta(hours=4)) is False

    @patch('apps.bookings.services.availability_service.WorkingHours.objects.get')
    def test_get_technician_availability(self, mock_get):
        """
        Document: booking-test-strategy.md
        Section: 5.1
        ...
        slots = AvailabilityService.get_technician_availability("1", target_date, actor)
        assert len(slots) == 1
    ...
    def test_update_working_hours(self, mock_event, mock_audit, mock_goc):
        actor = MagicMock(role=Roles.MANAGER)
        mock_wh = MagicMock(schedule_blob={})
        mock_goc.return_value = (mock_wh, False)
        
        new_blob = {"tuesday": {"active": True}}
        AvailabilityService.update_working_hours("1", new_blob, actor)
    ...
    def test_create_blackout_date(self, mock_event, mock_audit):
        actor = MagicMock(role=Roles.MANAGER)
        now = timezone.now()
        
        blackout = AvailabilityService.create_blackout_date("1", now, now + timedelta(days=1), actor)
        
        assert blackout.technician_id == "1"
        mock_audit.assert_called_once()
    ...
    def test_delete_blackout_date(self, mock_event, mock_audit, mock_get):
        actor = MagicMock(role=Roles.MANAGER)
        mock_blackout = MagicMock(technician_id="1")
